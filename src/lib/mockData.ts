...
export function generateMockData(
  ...
  const randomFn = seededPseudoRandom(indicator.id + (indicator.apiIdentifier || ''));
  ...
  else if (indicator.id === 'PMI' || indicator.apiIdentifier === 'PMI_MANUFACTURING_INVESTING_MOCK') {
    baseValue = 50 + (randomFn() - 0.5) * 10;
    volatility = 0.5 + randomFn() * 1.5;
  } else if (indicator.id === 'PMI_SERVICES' || indicator.apiIdentifier === 'PMI_SERVICES_INVESTING_MOCK') {
    baseValue = 52 + (randomFn() - 0.5) * 10;
    volatility = 0.5 + randomFn() * 1.5;
  } else if (indicator.id === 'PENDING_HOME_SALES' || indicator.apiIdentifier === 'PENDING_HOME_SALES_INVESTING_MOCK') {
    baseValue = 100 + (randomFn() - 0.5) * 30;
    volatility = 2 + randomFn() * 3;
  } else if (indicator.id === 'CCI' || indicator.apiIdentifier === 'CCI_INVESTING_MOCK') {
    baseValue = 100 + (randomFn() - 0.5) * 40;
    volatility = 3 + randomFn() * 4;
  }
  ...
  for (const date of dates) {
    ...
    if (indicator.id === 'PMI' || indicator.id === 'PMI_SERVICES' || indicator.apiIdentifier === 'PMI_MANUFACTURING_INVESTING_MOCK' || indicator.apiIdentifier === 'PMI_SERVICES_INVESTING_MOCK') {
        currentValue = Math.max(35, Math.min(65, currentValue));
    } else if (indicator.id === 'UNRATE' || indicator.id === 'U6RATE') {
        currentValue = Math.max(1, Math.min(15, currentValue));
    } else if (indicator.id === 'CRYPTO_FEAR_GREED') {
        currentValue = Math.max(0, Math.min(100, Math.round(currentValue)));
    } else if (indicator.id === 'CAPUTIL') {
        currentValue = Math.max(50, Math.min(90, currentValue));
    } else if (indicator.id === 'CCI' || indicator.id === 'UMCSENT' || indicator.apiIdentifier === 'CCI_INVESTING_MOCK') {
        currentValue = Math.max(40, Math.min(150, currentValue));
    } else if (indicator.id === 'LEI') {
        currentValue = Math.max(80, Math.min(130, currentValue));
    } else if (indicator.id === 'PENDING_HOME_SALES' || indicator.apiIdentifier === 'PENDING_HOME_SALES_INVESTING_MOCK') {
        currentValue = Math.max(60, Math.min(140, currentValue));
    } else if (!indicator.unit.includes('%') && !indicator.id.includes('BALANCE') && !indicator.id.includes('SPREAD')) {
...
export async function fetchIndicatorData(
    ...
    if (indicator.calculation === 'YOY_PERCENT' && effectiveDateRangeForApiCall.startDate && indicator.apiSource !== 'ApiNinjas') {
        try {
    ...
    if (indicator.apiSource === 'Mock') {
        ...
        apiFetchAttempted = true; // Considered "attempted" for mock logic
          ...
          switch (indicator.apiSource) {
            ...
            case 'ApiNinjas':
              if (indicator.apiIdentifier) {
                ...
                // No mock supplementation here; if it fails, fallback logic below handles it.
                if (rawFetchedData.length > 0) {
                    ...
                    // console.warn(`[Orchestrator] ApiNinjas (latest price) for ${indicator.id} returned no data.`);
                }
              }
              break;
            ...
            case 'AlternativeMeAPI':
              ...
              break;
            // Mock case is handled before the switch now
            default:
    ...
    if (indicator.apiSource === 'Mock') {
        shouldUseFullMock = rawFetchedData.length === 0; // If even explicit mock somehow failed to generate
    } else if (apiFetchAttempted && (rawFetchedData.length === 0 || apiErrorOccurred)) {
        shouldUseFullMock = true;
    } else if (!apiFetchAttempted && indicator.apiSource !== 'Mock') { // Should be rare
        shouldUseFullMock = true;
    }
