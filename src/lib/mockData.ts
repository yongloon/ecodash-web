...
import {
    ...
    fetchDbNomicsSeries // <<< ENSURE THIS IS IMPORTED
} from './api';
import {
    calculateYoYPercent,
    calculateMoMPercent,
...
export function generateMockData(
  ...
  if (indicator.frequency === 'Monthly') adjustedStartDateForInterval = startOfMonth(actualStartDate);
  let dates: Date[] = [];
   try {
       const intervalEnd = actualEndDate >= adjustedStartDateForInterval ? actualEndDate : adjustedStartDateForInterval;
       if (adjustedStartDateForInterval > intervalEnd) { /* Warn */ } else {
   ...
   } catch (e) { /* Error, fallback */ try { if (actualStartDate <= actualEndDate) dates = eachDayOfInterval({ start: actualStartDate, end: actualEndDate }); } catch (fbE) { return []; } }
  if (dates.length === 0) return [];
  let baseValue = 100; let volatility = 5;
  if (indicator.id === 'SP500' || indicator.id === 'BTC_PRICE_USD') { baseValue = (indicator.id === 'SP500' ? 4500 : 40000) + randomFn() * (indicator.id === 'SP500' ? 1000 : 10000); volatility = (indicator.id === 'SP500' ? 50 : 2000); }
  ...
  for (const date of dates) {
    const trend = 0.01 * volatility * (randomFn() - 0.4); const noise = (randomFn() - 0.5) * volatility; currentValue += trend + noise;
    if (!indicator.unit.includes('%') && !indicator.id.includes('BALANCE') && !indicator.id.includes('SPREAD') && indicator.id !== 'CRYPTO_FEAR_GREED') currentValue = Math.max(currentValue, 0.01);
...
export async function fetchIndicatorData(
    ...
    // ... (date range logic and YoY adjustment logic from previous step remains the same) ...
        ...
        try {
          ...
          effectiveDateRangeForApiCall.startDate = format(subYears(originalStartDate, 1), 'yyyy-MM-dd');
           if (parseISO(effectiveDateRangeForApiCall.startDate) >= parseISO(effectiveDateRangeForApiCall.endDate)) {
                effectiveDateRangeForApiCall.endDate = format(subYears(parseISO(actualDateRangeToUse.endDate), -1), 'yyyy-MM-dd');
           }
        } catch (e) { effectiveDateRangeForApiCall = { ...actualDateRangeToUse }; }
    }
    ...
    try {
      switch (indicator.apiSource) {
        ...
        case 'AlphaVantage':
          ...
          break;
        case 'DBNOMICS': // <<< ENSURE THIS CASE IS PRESENT
          if (indicator.apiIdentifier) { rawFetchedData = await fetchDbNomicsSeries(indicator.apiIdentifier, actualDateRangeToUse); apiFetchAttempted = true; }
          break;
        case 'CoinGeckoAPI':
        ...
        case 'AlternativeMeAPI':
          ...
          break;
        case 'Mock': /* Handled by fallback */ break;
        default: /* Fallback for unknown or FRED-like */
    ...
    } catch (error) { apiErrorOccurred = true; console.error(`[Orchestrator] API call error for ${indicator.id}:`, error); }

    const useMock = indicator.apiSource === 'Mock' || (apiFetchAttempted && (rawFetchedData.length === 0 || apiErrorOccurred)) || (!apiFetchAttempted && indicator.apiSource !== 'Mock');
    if (useMock) {
      if (indicator.apiSource !== 'Mock') console.warn(`[Orchestrator] Fetch fail/empty for ${indicator.id}, using mock.`);
    ...
    if (indicator.calculation && indicator.calculation !== 'NONE' && processedData.length > 0) {
        let calcSuccess = true; let tempCalc: TimeSeriesDataPoint[] = [];
        switch (indicator.calculation) {
          case 'YOY_PERCENT': let lookback = 12; if (indicator.frequency === 'Quarterly') lookback = 4; else if (indicator.frequency === 'Weekly') lookback = 52; else if (indicator.frequency === 'Daily') lookback = 365; tempCalc = calculateYoYPercent(processedData, lookback); break;
          case 'MOM_PERCENT': tempCalc = calculateMoMPercent(processedData); break;
          case 'QOQ_PERCENT': tempCalc = calculateQoQPercent(processedData); break;
          case 'MOM_CHANGE':  tempCalc = calculateMoMChange(processedData); break;
          default: calcSuccess = false; tempCalc = processedData; break;
        }
        processedData = tempCalc; if (calcSuccess) console.log(`[Orchestrator] ${indicator.id} after ${indicator.calculation}: ${processedData.length} points`);
    }
