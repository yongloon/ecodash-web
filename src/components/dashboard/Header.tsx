...
import { 
    ...
    Menu as HamburgerIcon, X as CloseIcon, Search // <<< ADDED Search
} from 'lucide-react'; 
...
import { CommandPalette } from '@/components/CommandPalette'; // <<< ADDED THIS
...
export default function Header({ toggleMobileMenu, isMobileMenuOpen }: HeaderProps) {
  ...
  // const userTier: AppPlanTier | undefined = currentUser?.activePlanTier; // Not directly used in header logic after this round
  
  const pathname = usePathname();
  const [isCommandPaletteOpen, setIsCommandPaletteOpen] = useState(false); // <<< ADDED THIS
  ...
  return (
    <>
      ...
      <CommandPalette open={isCommandPaletteOpen} setOpen={setIsCommandPaletteOpen} /> {/* <<< ADDED THIS */}
    </>
  );
}