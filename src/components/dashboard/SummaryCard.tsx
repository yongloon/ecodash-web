...
import { IndicatorMetadata, TimeSeriesDataPoint, indicatorCategories } from '@/lib/indicators'; // Ensure indicatorCategories is imported
...
export default function SummaryCard({ indicator, latestValue, previousValue, sparklineData }: SummaryCardProps) {
  const displayValue = latestValue?.value !== null && latestValue?.value !== undefined
    ? `${latestValue.value.toLocaleString(undefined, { 
        minimumFractionDigits: indicator.unit === '%' || indicator.unit === 'Index (0-100)' ? 1 : 0, // More precision for %/Index
  ...
  let trendColor = 'text-muted-foreground'; // Default color for neutral or N/A trend
  ...
  if (latestValue?.value !== null && latestValue?.value !== undefined && 
    ...
    if (latestValue.value > previousValue.value) {
      TrendIcon = FaArrowUp;
      trendColor = 'text-[hsl(var(--chart-green))]'; // Use CSS variable for trend color
    } else if (latestValue.value < previousValue.value) {
      TrendIcon = FaArrowDown;
      trendColor = 'text-[hsl(var(--chart-red))]'; // Use CSS variable
    }
  }

  const categoryData = Object.values(indicatorCategories).find(cat => cat.key === indicator.categoryKey);
  const linkHref = categoryData 
    ? `/category/${categoryData.slug}?indicator=${indicator.id}` 
    : `/`; // Fallback to overview or a default category

  let formattedDate = '';
  if (latestValue?.date && isValid(parseISO(latestValue.date))) {
      try { formattedDate = `(${format(parseISO(latestValue.date), 'MMM d, yyyy')})`; } catch (e) {} // More specific date
  }

  const validSparklineData = sparklineData?.filter(d => d.value !== null && typeof d.value === 'number' && d.date && isValid(parseISO(d.date))) || [];
  
  let sparklineStrokeColor = "hsl(var(--chart-neutral))"; // Default from CSS variables
  ...
  const gradientId = `sparklineGradient-${indicator.id.replace(/[^a-zA-Z0-9]/g, '')}`;
  ...
  return (
     <Link href={linkHref} className="block hover:shadow-xl focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background transition-shadow duration-200 rounded-lg group">
        <Card className="h-full border border-border bg-card text-card-foreground group-hover:border-primary/60 transition-colors duration-200 flex flex-col">
            <CardHeader className="pb-2 px-4 pt-4 sm:px-5 sm:pt-5"> {/* Consistent padding */}
            ...
            <CardContent className="pt-0 pb-3 px-4 sm:px-5 flex-grow flex flex-col justify-end">
                {validSparklineData.length > 1 ? (
                    <div className="w-full h-[40px] sm:h-[50px] mb-1 -mx-1"> {/* Negative margin to make it bleed slightly if desired, or remove */}
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={validSparklineData} margin={{ top: 5, right: 0, left: 0, bottom: 0 }}>
                                <defs>
                                    <linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1">
                                        <stop offset="5%" stopColor={sparklineFillColor} stopOpacity={0.3}/>
                                ...
                                <YAxis domain={['auto', 'auto']} hide padding={{ top: 5, bottom: 5 }}/> 
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                ) : (
